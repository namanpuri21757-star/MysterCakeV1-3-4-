...
python -m provenance_tool.cli init-db --db demo.db

:: 3. Snapshot policies for a domain
echo Snapshotting policies for example.com...
python -m provenance_tool.cli snapshot-policies --db demo.db --dataset-id raw_data_v1 --url https://example.com
...
python -m provenance_tool.cli generate-report --db demo.db --artifact-id clean_data_v1 --out report.json
